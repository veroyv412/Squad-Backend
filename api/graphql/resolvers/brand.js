...
const getBrands = async (root, args, context, info) => {
  ...
  if (brands.length > 0) {
    for (let brand of brands) {
      ...
      brand.banner =
        brand.customer && brand.customer.length > 0 ? brand.customer[0].companyBanner : '';
...
const subscribeToBrand = async (parent, args) => {
  await dbClient
    .db(dbName)
    .collection('brand_subscriptions')
    .insertOne({ userId: new ObjectId(args.userId), brandId: new ObjectId(args.id) });
