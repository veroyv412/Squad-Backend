...
const getFilterLooksByBrandCategoryProduct = async (root, args, context, info) => {
  ...
  if (userId) {
    totalUploads = await dbClient
      .db(dbName)
      .collection('uploads')
      .find({ memberId: new ObjectId(userId) })
      .count();
  }
  ...
  if (brandId && brandId != '-') {
    ...
    const brandUploads = await dbClient
      .db(dbName)
      .collection('uploads')
      .aggregate([
        {
  ...
  if (categoryId && categoryId != '-') {
    ...
    const categoryUploads = await dbClient
      .db(dbName)
      .collection('uploads')
      .aggregate([
        {
  ...
  if (productId && productId != '-') {
    ...
    const productUploads = await dbClient
      .db(dbName)
      .collection('uploads')
      .aggregate([
        {
  ...
  if (location && location != '-') {
    userFind.push({
      $or: [
        { 'user.hometownCity': new RegExp(location) },
        { 'user.hometownState': new RegExp(location) },
      ],
    });
  }
  ...
  if (userFind.length) {
    ...
    const memberUploads = await dbClient
      .db(dbName)
      .collection('uploads')
      .aggregate([
        {
...
const getFilterMemberByGenderAgeLocation = async (root, args, context, info) => {
  ...
  if (gender) {
    const usersGender = await dbClient
      .db(dbName)
      .collection('users')
      .aggregate([
    ...
    const usersGenderList = await dbClient
      .db(dbName)
      .collection('users')
      .aggregate([{ $match: { gender: gender, role: 'member' } }])
      .toArray();
  ...
  if (location) {
    const usersLocation = await dbClient
      .db(dbName)
      .collection('users')
      .aggregate([
    ...
    const usersLocationList = await dbClient
      .db(dbName)
      .collection('users')
      .aggregate([{ $match: { locationCity: new RegExp(location), role: 'member' } }])
      .toArray();
  ...
  if (ageFrom && ageTo) {
    const usersAgeCount = await dbClient
      .db(dbName)
      .collection('users')
      .find({ age: { $gte: ageFrom, $lte: ageTo }, role: 'member' })
      .count();
    const usersAgeCountList = await dbClient
      .db(dbName)
      .collection('users')
      .find({ age: { $gte: ageFrom, $lte: ageTo }, role: 'member' })
      .toArray();
...
const getFilterCustomerId = async (root, args, context, info) => {
  ...
  if (customerId) {
    const customerCredits = await dbClient
      .db(dbName)
      .collection('customer_credits')
      .aggregate([
        {
    ...
    if (customerCredits.length > 0) {
      result.data.push({
        name: customerCredits[0]._id.companyName,
        count: customerCredits[0].customerCount,
      });
    } else {
