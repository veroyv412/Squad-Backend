...
const stitchClient = Stitch.initializeDefaultAppClient(process.env.REALM_APP_NAME);
...
const getGroups = async (customerId) => {
  const customerGroups = await dbClient
    .db(dbName)
...
const getFeedbackAnswers = async (root, args, context, info) => {
  let customerFeedback = await dbClient
    .db(dbName)
    .collection('customer_feedback')
    .aggregate([
      {
  ...
  let customerFeedbackAnswers = await dbClient
    .db(dbName)
...
const getCustomerQuestions = async (root, args, context, info) => {
  let customerQuestions = await dbClient
    .db(dbName)
    .collection('customer_questions')
    .aggregate([
      {
...
const getCreditHistory = async (root, args, context, info) => {
  let creditHistory = await dbClient
    .db(dbName)
...
const saveCustomer = async (parent, args) => {
  try {
    ...
    if (brand.length > 0) {
      ...
      brand = await dbClient
        .db(dbName)
        .collection('brands')
        .insertOne({ name: customerInput.companyBrand, verified: false });
    ...
    let customerBrand = await dbClient
      .db(dbName)
      .collection('customer_brands')
      .findOne({ brandId: customerInput.brandId });
    if (!customerBrand) {
      await dbClient
        .db(dbName)
        .collection('customer_brands')
        .insertOne({
          customerId: new ObjectId(lastId),
          brandId: customerInput.brandId,
          name: customerInput.companyBrand,
        });
    }
...
const verifyCustomer = async (parent, args) => {
  try {
    ...
    const customerCategories = await dbClient
      .db(dbName)
      .collection('customer_categories')
      .find({ customerId: new ObjectId(args.id) })
      .toArray();
...
const saveFeedback = async (parent, args) => {
  try {
    ...
    const customer = await dbClient
      .db(dbName)
      .collection('customers')
      .findOne({ _id: new ObjectId(args.data.customerId) });
    const availableCredits = customer.availableCredits
      ? customer.availableCredits - totalAmount
      : 0;
    ...
    //Add Notification
    await notificationResolvers.helper.createOfferNotificationsToMembers(
      args.data.customerId,
      feedBack.uploads,
      _feedback.insertedId.toString()
    );
...
const addCredit = async (parent, args) => {
  try {
    ...
    const customer = await dbClient
      .db(dbName)
      .collection('customers')
      .findOne({ _id: new ObjectId(args.customerId) });
    const availableCredits = customer.availableCredits
      ? customer.availableCredits + args.amount
      : args.amount;
    await dbClient
      .db(dbName)
      .collection('customers')
