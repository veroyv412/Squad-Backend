...
const saveProduct = async (parent, args) => {
  try {
    let photo = {
      ...
      brandId:
        args.product.brand && args.product.brand._id ? new ObjectId(args.product.brand._id) : null,
      categoryId:
        args.product.category && args.product.category._id
          ? new ObjectId(args.product.category._id)
          : null,
    ...
    if (!photo.brandId) {
      ...
      if (!brand) {
        ...
        await dbClient.db(dbName).collection('customer_brands').insertOne({
          customerId: photo.customerId,
          brandId: photo.brandId,
          name: args.product.brand.name.toLowerCase(),
        });
      }
    }
    ...
    if (!photo.categoryId) {
      ...
      if (!category) {
        let category = await dbClient.db(dbName).collection('categories').insertOne({
          name: args.product.category.name,
          verified: true,
          createdAt: new Date(),
          updatedAt: new Date(),
        });
        ...
        await dbClient.db(dbName).collection('customer_categories').insertOne({
          ...
          name: args.product.category.name.toLowerCase(),
        });
      }
    }
